...
def show(): 
    ...
    # --- Config ---
    st.set_page_config(page_title="Keeper Pulse | Customer Health & Insights", layout="wide")
    st.title("🧩 Keeper Pulse: Health Stages & Signals")
    ...
    with st.container():
        ...
        with filter_col:
            selected_fom = st.selectbox("", df_score_summary['FOM_str'].unique()[::-1], label_visibility="collapsed")
    ...
    with left_col:
        ...
        fig1.update_layout(showlegend=False, plot_bgcolor='white', xaxis=dict(showgrid=False), yaxis=dict(showgrid=False))
        ...
        with chat_col2:
            ...
            user_input = st.text_area("##### 🤖 Ask Keeper", value=default_q, height=5)
        ...
        df_table = df_fom[['account_id', 'activity_score_label', 'support_score_label', 'usage_score_label', 'score_final', 'stage']].copy()
