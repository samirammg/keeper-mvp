...
# --- Config ---
st.set_page_config(page_title="Account Overview", layout="wide")
# Load and clean data
...
with col_filters:
    ...
    filtered_df = df_contract.copy()

    account_ids = filtered_df['account_id'].unique()
    selected_account_id = st.selectbox("Select Account ID", account_ids, key="account_id_selector")

    st.markdown("</div>", unsafe_allow_html=True)

if selected_account_id:
    account_df = filtered_df[filtered_df['account_id'] == selected_account_id]
    ...
    if not account_df.empty:
        ...
        with col_left:
            ...
            with sub_col1:
                ...
                <div style='border: 1px solid #d3d3d3; border-radius: 10px; padding: 20px;'>
                    <table style='width: 100%; font-size: 16px;'>
                        ...
                        <tr><td><b>Revenue</b></td><td>${revenue/1e6:.1f}M</td></tr>
            ...
            with sub_col2:
                ...
                df = df_score[df_score['account_id'] == selected_account_id].copy()
        ...
        df_score_filtered = df_score[df_score['account_id'] == selected_account_id].copy()
        ...
        with col_right:
            st.markdown("<div style='border: 1px solid #d3d3d3; border-radius: 10px; padding: 15px;'><h4>Risk Score</h4><p style='font-size: 34px; font-weight: bold;'>" + str(int(round(df_score[df_score['account_id'] == selected_account_id].sort_values(by='fom').iloc[-1]['score_final'] * 100))) + "</p></div>", unsafe_allow_html=True)
 ...
 # Get the latest usage score for the selected account
               # Get the latest usage score for the selected account
            latest_usage = df_score[df_score['account_id'] == selected_account_id].sort_values(by='fom').iloc[-1]
           ...
           # Background gradient (manually define segments)
            ...
            ax3.set_title("Product Usage", va='top', pad=10, fontsize=6, fontweight='bold')
            alfa= -np.pi/2
            # Needle
            angle = np.pi * ((latest_usage['usage_score'] + 1) / 2)+alfa
            a3=angle
            ...
            # --- Support Score Gauge ---
            latest_support = df_score[df_score['account_id'] == selected_account_id].sort_values(by='fom').iloc[-1]
            ...
            ax4.set_title("Support", va='top', pad=10, fontsize=6, fontweight='bold')
            angle = np.pi * ((latest_support['support_score'] + 1) / 2)+alfa
            a4=angle
            ...
            latest_activity = df_score[df_score['account_id'] == selected_account_id].sort_values(by='fom').iloc[-1]
            ...
            ax5.set_title("CSM Activity", va='top', pad=10, fontsize=6, fontweight='bold')
            angle = np.pi * ((latest_activity['activity_score'] + 1) / 2)+ alfa
            ...
            with col_left:
                ...
                keeper_input = st.text_area("", value=default_question, height=100)
                st.button("Ask Keeper")
