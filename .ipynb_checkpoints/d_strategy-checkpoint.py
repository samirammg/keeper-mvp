...
def show(): 
    ...
    with st.container():
        ...
        with col3:
            menu = st.selectbox("", [
                "📈 Stay on Strategy Page",
                "📌 CSM Activity & Insights",
                "💬 Support Trends & Insights",
                "📊 Product Usage & Behavior",
                "🧩 Keeper Pulse: Health Stages & Signals",
                "📘 Customer Journey Tracker"
            ], label_visibility="collapsed")

    if menu == "📌 CSM Activity & Insights":
        switch_page("d_activity")
    elif menu == "💬 Support Trends & Insights":
        switch_page("d_sup")
    elif menu == "📊 Product Usage & Behavior":
        switch_page("d_usage")
    elif menu == "🧩 Keeper Pulse: Health Stages & Signals":
        switch_page("d_pls")
    elif menu == "📘 Customer Journey Tracker":
        switch_page("d_account")
    ...
    with col_filter:
        selected_month = st.selectbox("First Order Month", month_options, index=month_options.index('Jan 2024'))
    try:
    ...
    with main_col:
     ...
     # --- Future Projection & Strategic Growth ---
        ...
        with col2:
            ...
            user_churn_input = st.number_input("Set Churn Rate (%)", min_value=0.0, max_value=1.0, value=default_churn, step=0.01, key="user_churn")
        
        with col3:
            default_acq = float(df_user_month['New Accounts Rate'].values[0]) if not df_user_month.empty else 0
            user_acq_input = st.number_input("Set Acquisition Rate (%)", min_value=0.0, max_value=1.0, value=default_acq, step=0.01, key="user_acq")
        ...
        with top_right:
            ...
            <div style="border: 2px solid #888; padding: 20px; border-radius: 8px; background-color: #FFFFFF;">
                    ...
                    <tr><td style="padding: 6px 12px;">Set Simulation Date</td><td>{user_selected_month}</td><td>{user_churn_input:.2%}</td><td>{user_acq_input:.2%}</td></tr>
                </table>
            </div>
            """, unsafe_allow_html=True)
        ...
        # --- Combine both simulations ---
            def run_combined_simulation(df, fom_start, new_churn_rate, new_acq_rate):
                df = df.copy()
                ...
                original_churn = df.loc[start_index, 'Churn Accounts Rate']
                churn_change = (new_churn_rate - original_churn) / original_churn if original_churn else 0
            
                original_acq = df.loc[start_index, 'New Accounts Rate']
                acq_change = (new_acq_rate - original_acq) / original_acq if original_acq else 0
                ...
                for i in range(start_index, len(df)):
                    ...
                    churn_rate_prev = df.loc[i - 1, 'Churn Accounts Rate_c']
                    churn_rate_updated = churn_rate_prev * (1 + churn_change)
                    df.loc[i, 'Churn Accounts Rate_c'] = churn_rate_updated
            
                    renew_c = df.loc[i, 'Renew Accounts'] * (1 - churn_change)
                    upsell_c = df.loc[i, 'Upsell Accounts'] * (1 - churn_change)
                    churn_c = df.loc[i, 'Churn Accounts'] * (1 + churn_change)
                    new_c = df.loc[i, 'New Accounts'] * (1 + acq_change)
                    active_c = df.loc[i, 'Active Accounts'] + new_c - churn_c+renew_c +upsell_c
            ...
            sim_result_final = run_combined_simulation(df, fom_start=user_selected_period, new_churn_rate=user_churn_input, new_acq_rate=user_acq_input)
